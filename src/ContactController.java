...
public class ContactController {
	...
	public static JScrollPane sortingByName(){
        ContactList contactList = DBConnection.getInstance().getContactList();
        Contact[] tempArray = new Contact[contactList.size()];
        for(int i=0;i<tempArray.length;i++){
            tempArray[i] = contactList.get(i);
        }
        for(int i=0;i<tempArray.length;i++){
            for(int j=i+1;j<tempArray.length;j++){
                if(tempArray[i].getName().compareTo(tempArray[j].getName())>0){
                    Contact temp = tempArray[i];
        ...
        for(int i=0;i<tempArray.length;i++){
            Object[] rowData={tempArray[i].getId(), tempArray[i].getName(), tempArray[i].getPhoneNumber(), tempArray[i].getCompanyName(), tempArray[i].getSalary(), tempArray[i].getBirthday()};
            dtm.addRow(rowData);
        }
